...
void geo(point Input p[1], inout TriangleStream<Input> tri) {
	...
	float4 v[8];
	v[0] = float4(nPos + size * r - size * up, 1.0f);
	v[1] = float4(nPos + size * r + size * up + wind, 1.0f);
	v[2] = float4(nPos - size * r - size * up, 1.0f);
	v[3] = float4(nPos - size * r + size * up + wind, 1.0f);

	v[4] = float4(nPos + size * f - size * up, 1.0f);
	v[5] = float4(nPos + size * f + size * up + wind, 1.0f);
	v[6] = float4(nPos - size * f - size * up, 1.0f);
	v[7] = float4(nPos - size * f + size * up + wind, 1.0f);
