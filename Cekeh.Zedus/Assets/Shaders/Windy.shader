...
Shader"Nature/WavyGrass (Windy.shader)" {
...
#pragma surface surf Standard fullforwardshadows alphatest:_Cutoff vertex:vert
	...
	void vert(inout appdata_full v) {
		...
		float4 _waveZmove = float4 (0.05 , .1,-0.05 , 0.1);// 0.006, .02, -0.02, 0.1
		...
		float waveAmount = (v.texcoord.y + (.5 - v.texcoord.y)) *(v.color.a + _ShakeBending);//added (.5 - v.texcoord.y)
